...
def get_top_hundred_anime(request, number):
    anime = sorted(Anime.objects.all(),
                   key=lambda anime: anime.rating, reverse=True)[:number]
    animeSerializer = AnimeSerializer(anime, many=True)
    return JsonResponse(animeSerializer.data, safe=False)
...
def get_favorites(request, username):
    user = User.objects.get(username=username)
    print(user)
    savedAnime = list(map(lambda x: x.anime_id, SavedAnime.objects.all().filter(
        user_id=user.id).filter(type='Favorite')))
...
def get_watch_later(request, username):
    ...
    print(user)
    savedAnime = list(map(lambda x: x.anime_id, SavedAnime.objects.all().filter(
        user_id=user.id).filter(type='Watch Later')))
    savedAnime = list(map(lambda x: Anime.objects.get(id=x), savedAnime))
    animeSerializer = AnimeSerializer(savedAnime, many=True)
    return JsonResponse(animeSerializer.data, safe=False)
...
def get_watched(request, username):
    ...
    print(user)
    savedAnime = list(map(lambda x: x.anime_id, SavedAnime.objects.all().filter(
        user_id=user.id).filter(type='Watched')))
    savedAnime = list(map(lambda x: Anime.objects.get(id=x), savedAnime))
    animeSerializer = AnimeSerializer(savedAnime, many=True)
    return JsonResponse(animeSerializer.data, safe=False)
...
def remove_anime_from_favorites(request, anime_id, username):
    ...
    anime_in_db, created = SavedAnime.objects.all().filter(
        user_id=user.id).filter(anime_id=anime.id).filter(type='Favorite').delete()
    return JsonResponse(data={}, status=status.HTTP_200_OK)
...
def remove_anime_from_watch_later(request, anime_id, username):
    ...
    anime_in_db, created = SavedAnime.objects.all().filter(user_id=user.id).filter(
        anime_id=anime.id).filter(type='Watch Later').delete()
    return JsonResponse(data={}, status=status.HTTP_200_OK)
...
def remove_anime_from_watched(request, anime_id, username):
    ...
    anime_in_db, created = SavedAnime.objects.all().filter(
        user_id=user.id).filter(anime_id=anime.id).filter(type='Watched').delete()
    return JsonResponse(data={}, status=status.HTTP_200_OK)
